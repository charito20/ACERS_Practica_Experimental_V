...
El hallazgo DR-01 no se corrige en esta sección porque su solución es la propia ampliación de la matriz de trazabilidad con las diez columnas de la plantilla, tarea liderada en el Paso 2; el estado queda registrado como abierto con responsable y fecha de cierre comprometida antes de la entrega.

Para verificar que las correcciones no introdujeron defectos nuevos, la re-inspección incluyó una pasada completa posterior sobre las fichas modificadas y sobre las métricas afectadas (M5 y M6 se recalculan sobre la versión corregida; véase la Sección~\ref{sec:metricas-calidad}). Los conteos base publicados en el anexo de auditoría corresponden todos a la versión corregida entregada.

\subsection{Veredicto de validación}
\label{sec:veredicto-validacion}

Tras el segundo ciclo, el ERS queda sin defectos críticos o mayores abiertos en su texto: el único hallazgo pendiente (DR-01) es de integración de artefactos, no de especificación, y tiene responsable y plazo. La tasa de defectos residuales resultante se reporta como M6 en la Sección~\ref{sec:metricas-calidad}, con el par antes/después exigido. Los cambios descritos quedan incorporados a la versión final consolidada del SRS que cierra esta práctica, con número de versión, fecha y commit asociado.
